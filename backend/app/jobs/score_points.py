...
Reads tags off Zoho Sprints-sourced tickets and applies the Engineering
Points tables (app/models/points_rules.py) to produce an append-only ledger
...
by a tag added long after the original scoring run — see app/models/points.py
for why this can't be a stateless recompute.

One run does two independent things over the SAME watermark window:

1. SCORE new candidates — closed tickets, tickets whose `triaged` tag was
...
2. ADJUST existing awards — a newly-added `reverted` tag reverses every
   ...
   is never mutated); a newly-added `rca-done` tag adds a `+4` `bonus` row
   on top of the ticket's `incident` award. Both are read from
   issue_tag_history (the append-only tag diff log Zoho ingestion writes),
   the same way this file discovers newly-triaged tickets.
...
async def notify_unscored(session) -> int:
    """Extension point for the (currently deferred) Slack ping.
