...
Additive alongside app/api/insights.py: ByActorResponse/ActorStat (Linear-
throughput-derived) are untouched. The frontend fetches both and merges
client-side, same as it already independently fetches overview/throughput/
by-actor today. Mounted at /api/insights/points/* so it rides the existing
Next.js catch-all proxy (frontend/app/api/insights/[...path]/route.ts) with
zero proxy changes.
